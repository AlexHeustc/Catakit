...
def get_OER(models, ReactionPath, G, pH):
    ...
    GOH = GH2O - GH2 / 2  # eV 1/2*H2 + OH- --> H2O
...
def get_HzOR(models, ReactionPath, G, pH):
    deltaG = -1.32  # eV  v + N2H4 + 4OH- --> N2 + 4H2O + 4e-
    # GN2 = GN2H4 - GH2 * 2 - deltaG  # eV
    # GO2 = GN2H4 - GH2 -GN2 - deltaG  # eV
    GOH = GH2O - GH2 / 2
    GN2H4 = GN2 + 2 * GH2 + deltaG  # eV
    G_U0_models = []  # Relatvie Gibbs free energies at U=0V
    G_UV_models = []  # Relatvie Gibbs free energies at U=1.23V
    # G_Ueta_models = []  # Relatvie Gibbs free energies at U=(1.23 + eta) V
    eta_models = []  # Overpotential
    for name in models:  # for each model
        # alkaline condition,use GH2 instead of GH2O - GOH
        deltaG_pH = (14 - pH) * kb * Temperature * log(10)
        i = models.index(name)
        G_U0_1 = G[i][1] - GN2H4 - G[i][0]  # v + N2H4 --> *N2H4
        G_U0_2 = G[i][2] - G[i][0] - GN2H4 + 1 / 2 * GH2 - deltaG_pH  # v + N2H4  --> *N2H3 + H+ + e-
        G_U0_3 = G[i][3] - G[i][0] - GN2H4 + GH2 - 2 * deltaG_pH  # v + N2H4  --> *N2H2 + 2H+ + 2e-
        G_U0_4 = G[i][4] - G[i][0] - GN2H4 + 3 / 2 * GH2 - 3 * deltaG_pH  # v + N2H4  --> *N2H + 3H+ + 3e-
        G_U0_5 = G[i][5] - G[i][0] - GN2H4 + 2 * GH2 - 4 * deltaG_pH  # v + N2H4   --> *N2 + 4H+ + 4e-
        G_U0_6 = GN2 + 2 * GH2 - GN2H4 - 4 * deltaG_pH  # v + N2H4  --> * + N2 + 4H+ + 4e-
        G_UV_1 = G_U0_1
        G_UV_2 = G_U0_2 + 1 * deltaG / 4
        G_UV_3 = G_U0_3 + 2 * deltaG / 4
        G_UV_4 = G_U0_4 + 3 * deltaG / 4
        G_UV_5 = G_U0_5 + 4 * deltaG / 4
        G_UV_6 = G_U0_6 + 4 * deltaG / 4
        eta = max(G_UV_2 - G_UV_1, G_UV_3 - G_UV_2, G_UV_4 - G_UV_3, G_UV_5 - G_UV_4, G_UV_6 - G_UV_5)
        print("The overpotential of model %s is %.2f V" % (models[i], eta))
        G_U0_models.append(np.array([0.0, G_U0_1, G_U0_2, G_U0_3, G_U0_4, G_U0_5, G_U0_6]))
        G_UV_models.append(np.array([0.0, G_UV_1, G_UV_2, G_UV_3, G_UV_4, G_UV_5, G_UV_6]))
        eta_models.append(eta)
...
def get_GOR(models, ReactionPath, G, pH):
    deltaG = -1.32  # eV  v + N2H4 + 4OH- --> N2 + 4H2O + 4e-
    # GN2 = GN2H4 - GH2 * 2 - deltaG  # eV
    # GO2 = GN2H4 - GH2 -GN2 - deltaG  # eV
    GOH = GH2O - GH2 / 2
...
def plot_Reaction(models, ReactionPath, y):
    ...
    TextLabel = True  # 是否添加坐标点对应的数值文本标签
    FontSize = 20  # 设置能量值文本大小
    Axis_FontSize = 18  # 设置XY轴标签及标题大小
    color = ["red", "purple", "orange", "green", "cyan", "blue", "brown", "pink", "yellow",
             "black"]  # add new colors if models > 10
    path_label = models
...
def main():
    Reaction = input(
        "Please input the type of eletrochemical reaction: \n [1] OER \n [2] ORR \n [3] NRR \n [4] HzOR \n  ")
    ...
    if Reaction == "1":
        ...
        ReactionPath = input(
            "Please input the intermediates of the Reaction: \n such as: slab NN NNH NNH2 N NH NH2 NH3 \n").split()
        models, G = get_Gibbs(ReactionPath)
        G_U0, G_UV = get_NRR(models, ReactionPath, G, pH)
        plot_Reaction(models, ReactionPath[:], G_UV)
    elif Reaction == "4":
        ReactionPath = ["slab", "N2H4", "N2H3", "N2H2", "N2H", "N2"]
        models, G = get_Gibbs(ReactionPath)
        G_U0, G_UV = get_HzOR(models, ReactionPath, G, pH)
        plot_Reaction(models, ReactionPath[:], G_UV)
    else:
...
if __name__ == '__main__':
    global kb, Temperature, GH2, GH2O, GNH3, GN2
